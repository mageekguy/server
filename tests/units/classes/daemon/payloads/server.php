				...
				->mock($socketPoller)
					->call('pollSocket')
						->withArguments($serverSocket1)
							->before($this->mock($events1)->call('onReadNotBlock')->withArguments($connectHandler1)->once())
								->once()
					->call('pollSocket')
						->withArguments($serverSocket2)
							->before($this->mock($events2)->call('onReadNotBlock')->withArguments($connectHandler2)->once())
								->once()
					->call('waitSockets')->once()
				->mock($events2)->call('onReadNotBlock')->withArguments($connectHandler2)->once()
